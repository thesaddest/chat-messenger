...
export default class UserService {
    ...
    static async changeAvatar(formData: FormData, filepath: string): Promise<AxiosResponse<string>> {
        //modify req.headers to pass the username to multer middleware, so we can have <filepath>/<file> in our s3 bucket
        ...
        return api.post<string>(`${USER_API.USER_ENTITY}/${USER_API.CHANGE_AVATAR}`, formData);
    }
}
